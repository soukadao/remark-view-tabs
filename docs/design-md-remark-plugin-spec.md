...
第一版では属性構文を HTML 属性へ変換しない。プラグインは属性文字列を対象 mdast node から取り除き、解析結果だけを `data.design.attrs` に保持する。
...
第一版では、属性を以下の mdast node に付与する。

| Markdown | mdast上の扱い | 属性付与先 |
|---|---|---|
| `## 見出し {type=layout}` | `heading` の末尾に属性用 text が入る | `heading` |
| `![alt](path){device=pc}` | `paragraph` の child に `image` と属性用 text が入る | `image` |
| table 直後の `{type=controls}` | `table` の次に属性だけの `paragraph` が入る | 直前の `table` |

image 属性は、同一 paragraph 内で `image` の直後に属性用 text が続く場合だけ image node に付与する。paragraph に複数の image がある場合、属性は直前の image にだけ付与する。

table 属性は、table node の直後に属性だけを含む paragraph がある場合に直前の table node へ付与する。この paragraph は viewer へ渡す mdast から取り除く。

heading 属性は、heading の末尾 text から属性文字列を取り除き、空白を trim したうえで heading node に付与する。
