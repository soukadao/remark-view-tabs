...
function parseTabsList(name, display, list, file, options) {
    const tabs = [];
    const seen = new Set();
...
function parsePseudoTabsStart(node) {
...
function attachRootTabs(tree, tabs) {
...
export function parseTabs(tree, file, options) {
    const tabs = [];
    for (let index = 0; index < tree.children.length; index += 1) {
        const node = tree.children[index];
        const contentNode = tree.children[index];
